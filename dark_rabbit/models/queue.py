...
class DarkRabbitQueue(models.Model):
    ...
    listen = fields.Boolean(
        index=True, help="If set, then this queue will be listened for new messages"
    )
    listen_exclusive = fields.Boolean(help="Do not allow other consumers on the queue")
    ...
    event_ids = fields.One2many(
        comodel_name="dark.rabbit.event", inverse_name="queue_id"
    )
